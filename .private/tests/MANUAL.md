...
- `~/Applications` in Finder holds **CS193V Development Environment** with the course icon, shown
  at the same size as its neighbours — **not** shrunk inside a grey rounded square. A grey box
  means the artwork stopped carrying its own squircle and 100px gutter; `make-icons.sh` refuses
  that input, so it means the committed `.icns` was regenerated by something else.
- `⌘Space`, type `CS193` — it appears, and opens.
- It appears in Spotlight's **Apps** view. (There is no Launchpad to check: Apple removed it in
  Tahoe, which is why the Applications folder and Spotlight are the whole of the macOS story.)
...
recreates deleted shortcuts is microsoft/wslg#484, which is about WSLg *GUI-app* links
regenerated by a running plugin, a different mechanism from the one-shot shortcut
`wsl --install` writes for a distribution. If the plain entry comes back, the choice to delete it
