...
if [ -z "$stage2_pushed" ]; then
    ...
    # ─── AND IT SURVIVES THE PIPE IT IS PUBLISHED BEHIND  (#297) ──────────────
    # THE INSTRUCTION IS `curl -fsSL URL | bash`, so the published bytes are delivered to bash
    # on STDIN -- and `bash -n` above says nothing about that. A script read from a pipe is
    # executed as it arrives, which is why the whole executable body sits inside one brace
    # group; a release that lost the brace would pass every check above it and still hand a
    # student a half-run installer on a dropped connection.
    #
    # --dev-manifest-hash IS THE SAFE WAY TO ASK. The verb dispatches above the download and
    # above the mktemp and every arm exits, so this runs the REAL published file, through the
    # REAL pipe, and installs nothing -- the same exemption installer-door leans on. It is
    # handed this repository's own tree, so the answer is checkable rather than merely non-empty.
    #
    # `bash -s --` IS WHAT CARRIES AN ARGUMENT THROUGH A PIPE. Without `-s` bash reads the
    # script from the ARGUMENT instead of stdin, which would quietly test nothing.
    # A TINY TREE OF OUR OWN, not the repository: manifest_hash refuses anything that is not a
    # file or a directory, and it hashes every byte it walks -- so pointing it at the working
    # tree would be both slow and hostage to a stray symlink or socket in somebody's checkout.
    # Two files answer the question, which is whether the piped copy computes what ours does.
    mkdir -p "$rel_tmp/tree/sub"
    printf 'one\n' > "$rel_tmp/tree/a.txt"
    printf 'two\n' > "$rel_tmp/tree/sub/b.txt"
    rel_tree_hash="$(bash "$PRIVATE/install-cs193v.sh" --dev-manifest-hash "$rel_tmp/tree" 2>/dev/null)"
    rel_piped_hash="$(cat "$rel_tmp/stage2.sh" | bash -s -- --dev-manifest-hash "$rel_tmp/tree" 2>/dev/null)"
    # THE LOCAL ANSWER FIRST, so a published file that cannot hash anything is told from a
