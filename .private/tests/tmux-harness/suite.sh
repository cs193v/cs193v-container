...
LBURL=http://localhost:3002/magic-link
LBTMP="$(mktemp -d)"
...
if hx_wait "$S3" '\+ NEW TAB' 12; then
  ...
  if hx_wait "$S3" 'Continue in Browser' 15; then
  ...
  # The server open-url started would otherwise hold a forwarded port for fifteen minutes,
  # and the next run of this suite takes ports from the same small list.
